...
const Actions = () => {
  ...
  useEffect(() => {
    ...
    const item2 = selectedSubLabel.filter(item => item.id === selected2)[0].name;
  ...
  return (
    <div className="actions-cover">
      <div className='actions-left'>
        <Action ref={firstSlideMenuRef} onOpen={() => secondSlideMenuRef?.current.close()} onSelect={(id) => handleSelect(id)} items={labels}></Action>
      </div>
      <div className='actions-center'>

      </div>
      <div className='actions-right'>
        <Action onSelect={(id) => handleSelect2(id)} onOpen={() => firstSlideMenuRef?.current.close()} ref={secondSlideMenuRef} items={selectedSubLabel}></Action>
      </div>
    </div>
  );
};
